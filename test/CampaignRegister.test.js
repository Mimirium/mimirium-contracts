...
contract('CampaignRegsiter', function (accounts) {
    ...
    before(async () => {
        ...
        let lastNumber = web3.eth.blockNumber;
        now = web3.eth.getBlock(lastNumber).timestamp;
    })
    ...
    describe("Creating Campaigns", function () {
        ...
        it("owner can create campaign with correct data", async () => {
            let tx = await campaigns.createCampaign(DataTypes.Survey, companyId, 0, 10, now, now + oneDay, {from: owner, value: web3.toWei(1, "ether")});
            assert.equal(tx.logs[0].event, "CampaignCreated");
        })

        it("non-owner can NOT create campaign with correct data", async () => {
            await assertThrows(campaigns.createCampaign(DataTypes.Survey, companyId, 0, 10, now, now + oneDay, {from: user1, value: web3.toWei(1, "ether")}));
        })

        it("owner can NOT create campaign without budget", async () => {
            await assertThrows(campaigns.createCampaign(DataTypes.Survey, companyId, 0, 10, now, now + oneDay, {from: owner, value: 0}));
        })

        it("owner can NOT create campaign with incorrect timing", async () => {
            await assertThrows(campaigns.createCampaign(DataTypes.Survey, companyId, 0, 10, now, now - oneDay, {from: owner, value: web3.toWei(1, "ether")}));
            await assertThrows(campaigns.createCampaign(DataTypes.Survey, companyId, 0, 10, now - oneDay, now + oneDay, {from: owner, value: web3.toWei(1, "ether")}));
        })

        it("owner can NOT create campaign from non-existing company", async () => {
            await assertThrows(campaigns.createCampaign(DataTypes.Survey, 0x00000000000000000000, 0, 10, now, now + oneDay, {from: owner, value: web3.toWei(1, "ether")}));
        })
    })
    ...
    describe("Listing", function () {
        ...
        it("campaign data can be retrieved", async () => {
            ...
            assert.equal(campaign[1], DataTypes.Survey);
            assert.equal(campaign[2], companyId);
            assert.equal(campaign[3], 0);
            assert.equal(campaign[4], 10);
            assert.equal(campaign[5], web3.toWei(1, "ether"));
            assert.equal(campaign[6], now);
            assert.equal(campaign[7], now + oneDay);
        })
    })
})